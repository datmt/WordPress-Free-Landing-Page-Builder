...
class VE_Widget_Manager extends VE_Manager_Abstract{
    ...
    function add_wrapper($widget){
        static $instance=0;
        $instance++;
        $id='ve-widget-'.$widget->ID.'-'.$instance;
        $width=ve_get_post_setting('screen_size',$widget->ID);
        if($widget->width){
            $width=$widget->width;
        }
        $height=0;
        ...
        $classes=array();
        if($alignment){
            $classes[]='ve_widget ve_widget_'.$alignment;
        }
        ...
        $widget->post_content=sprintf('<div id="%s"%s%s>%s</div>',$id,$class,$style,$widget->post_content);
        return $widget;
    }
