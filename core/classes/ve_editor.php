...
class VE_Editor extends VE_Manager_Abstract{
    ...
    function setupPostToEdit() {
        ...
        $post_type=ve_get_param('post_type');
        if(!$this->post_id){
            wp_redirect(add_query_arg('post_type',$post_type,admin_url('edit.php')));
            die;
        }

        $this->post = get_post( $this->post_id ,OBJECT, 'edit');
    ...
    function do_element($element,$content=''){
        if(!empty($element['shortcode'])){
            preg_match( '/' . self::getShortCodeRegex() . '/', $element['shortcode'], $found );
            $element['attrs_query']='';
            $element['attrs']=array();
            if(isset($found[3])) {
                $element['attrs_query'] = stripslashes($found[3]);
                $element['attrs'] = shortcode_parse_atts($element['attrs_query']);
            }
    ...
    function element_wrap_start($element){
        $elementObj=$this->elementManager->getElement($element['id_base']);
        $custom_wrapper_class=$elementObj->getEditWrapperClass($element['attrs']);
        if(!empty($custom_wrapper_class)){
            $custom_wrapper_class=' '.trim($custom_wrapper_class);
        }
        return '<div class="ve_element'.$custom_wrapper_class.'" data-id-base="' . $element['id_base'] . '" data-element-id="' . $element['id'] . '"'  . '>';
    }
    function element_wrap_end($element){
        return '</div>';
    }
