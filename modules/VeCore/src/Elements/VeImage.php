...
class VeCore_VeImage extends Ve_Element implements VE_Element_Interface
{
    ...
    function init(){
	    ...
	    $this->ready('if (typeof ve_front !== "undefined"){ve_front.image.start();}');
    ...
    function form($instance){
        ...
        ?>
        <p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:'); ?></label> <input class="" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" /></p>
        <div class="ve_input_block">
            ...
            <div class="ve-media-selected-images">
                <ul class="ve-media-selected-images-list">
                    <?php echo ve_field_list_images($img_id);?>
                </ul>
            </div>
...
<!--            <span class="ve_description ve_clearfix">Enter image size. Example: "thumbnail", "medium", "large", "full" or other sizes defined by current theme. Alternatively enter image size in pixels: 200x100 (Width x Height). Leave empty to use "thumbnail" size.</span>-->
        ...
        <p class="small-input" id="custom_size_setting">
            ...
            <input class="" type="text" name="<?php $this->field_name('width');?>" value="<?php echo esc_attr($width);?>"> <br />
        ...
        <p data-show-if="<?php $this->field_id('img_link_large');?>" data-show-value=":checked" data-compare="!=">
            ...
            <input class="" type="text" placeholder="http://" name="<?php $this->field_name('link');?>" id="<?php $this->field_id('link');?>" value="<?php echo $link;?>"/>
        </p>
        <p><label for="<?php echo $this->get_field_id('class'); ?>"><?php _e('Extra class:'); ?></label> <input class="" id="<?php echo $this->get_field_id('class'); ?>" name="<?php echo $this->get_field_name('class'); ?>" type="text" value="<?php echo $class; ?>" /></p>
        <?php
    }
    function export($instance,$content=''){
        if(isset($instance['image'])){
            if($image_src=wp_get_attachment_image_src($instance['image'],'full')){
                $instance['image_src']=$image_src[0];
            }
        }
        return parent::export($instance,$content);
    }
    function import($instance,$content=''){
        global $ve_importing_post;
        if(!isset($ve_importing_post)){
            $ve_importing_post=0;
        }
        $ve_importing_post=absint($ve_importing_post);
        if(isset($instance['image_src'])){
            if($image=$this->download_image($instance['image_src'],$ve_importing_post)){
                $instance['image']=$image;
                unset($instance['image_src']);
            }
        }
        return parent::import($instance,$content);
    }
    function download_image( $file, $post_id, $desc = null ) {
        if ( ! empty( $file ) ) {
