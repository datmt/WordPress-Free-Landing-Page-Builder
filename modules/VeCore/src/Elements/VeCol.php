...
class VeCore_VeCol extends Ve_Element implements VE_Element_Interface{
    ...
    function convertWidth($width)
    {
        ...
        if ($width_parts[1] != "12")
        {
            ...
            $width_parts[0] = intval((12 * intval($width_parts[0])) / intval($width_parts[1]));
            $width_parts[1] = 12;
        }

        return "{$width_parts[0]}/{$width_parts[1]}";
    }
    ...
    function form($instance,$content=''){
        ...
        $instance["width"] = $this->convertWidth($instance["width"]);
        ?>
        <p>
