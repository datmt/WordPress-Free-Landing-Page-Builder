...
class VeCore_CssEditor extends Ve_Feature_Abstract{
        ...
        if($instance) {
            foreach ($instance as $style_name => $val) {
                ...
                if($style_name=='background-image'){
                    if(is_numeric($val)){
                        list($val,$width,$height)=wp_get_attachment_image_src($val, 'large');
                    }
                    if($val) {
    ...
    function form($instance) {
        ...
        $output .= '<div class="ve_col-xs-5 ve_settings">'
            . '    <h4>' . __( 'Border', 'visual_editor' ) . '</h4> '
            . '    <div class="border-settings">'
            . '         <div class="color-group"><label class="label">Color:</label> <input type="text" name="border_color" value="'.$this->get_field_value('border_color').'" class="ve_color-control"></div>'
            . '         <div class="ve_border-style"><label class="label">Style:</label> <select name="border_style" class="ve_border-style">' . $this->getBorderStyleOptions() . '</select></div>'
            .      '</div>'
            . '    <h4>' . __( 'Background', 'visual_editor' ) . '</h4>'
            . '    <div class="background-settings">'
            . '        <input type="radio" name="css-bg-type" value="solid-bg"  checked="checked"/> Solid &nbsp; &nbsp          '    
            . '        <input type="radio" name="css-bg-type" value="image-bg" /> Image          '    
            // image bg
            . '         <div class="css-bg-image css-bg-type">'
            ...
            . '         </div>'
            // solid color bg
            . '         <div class="css-bg-solid css-bg-type">'
